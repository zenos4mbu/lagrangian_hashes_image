...
class ConfigBaseTrainer:
    ...
    """
    pos_lr_weight : float = 1.0
    """ Learning rate weighting applied only for the pos and stds Gaussians parameters
        (e.g. parameters which contain "grid" in their name)
    """
    var_lr_weight : float = 1.0
    """ Learning rate weighting applied only for the pos and stds Gaussians parameters
        (e.g. parameters which contain "grid" in their name)
    """
    ...
    valid_metrics : Tuple[str, ...] = ('psnr','ssim' ) # lpips, ssim are also supported
    ...
    def init_optimizer(self):
        ...
        for name in params_dict:
            if 'decoder' in name:
                ...
                pos_params.append(params_dict[name])
            elif 'var' in name:
                var_params.append(params_dict[name])
            else:
        ...
        params.append({"params": pos_params, "eps": eps, "lr": lr * self.cfg.pos_lr_weight})
        params.append({"params": var_params, "eps": eps, "lr": lr * self.cfg.var_lr_weight})
