...
class MultiviewTrainer(BaseTrainer):
    ...
    def step(self, data):
        ...
        rb = self.pipeline(rays=rays, lod_idx=lod_idx, channels=["rgb", "density"])
        ...
        codebook_np = self.pipeline.nef.grid.codebook.pos
        ...
        #     # Select 16 samples from rb.density
        #     # sample_idx = torch.randint(0, rb.density.shape[0], (16,))
        #     sigma_loss = ((rb.total_gau.sum(dim=1)/8 * rb.density.squeeze()).sum())/653
        #     # sigma_loss = ((rb.total_gau.sum()/num_levels) * rb.density.squeeze()).sum()
        #     loss -= sigma_loss * 0.01

        # Position loss
        # with torch.cuda.amp.autocast():
        #     var_pos = self.pipeline.nef.grid.codebook.pos
        #     positions = var_pos[:, 1:] 
        #     loss_pos = torch.mean(torch.relu(positions - 1.1)) + torch.mean(torch.relu(-1.1 - positions))
        #     loss += loss_pos * 0.01
        ...
        with torch.cuda.amp.autocast():
            for level in range(num_levels):
                ...
                level_var = codebook_np[start_idx:end_idx, 0]
    ...
    def log_console(self):
        ...
        if self.epoch % 20 == 1:
            codebook_np = self.pipeline.nef.grid.codebook.pos.cpu().detach().numpy()
            begin_idxes = self.pipeline.nef.grid.codebook.begin_idxes_pos
            # Number of levels
            num_levels = len(begin_idxes) - 1
            for level in range(num_levels):
                    start_idx = begin_idxes[level]
                    end_idx = begin_idxes[level + 1]
                    positions = codebook_np[start_idx:end_idx, 1:]
                    with open( "{}/gaussian_positions_lv{:02d}_epoch_{:05d}.obj".format(self.tracker.log_dir,level,self.epoch), 'w') as f:
