...
# import splash._C as splash_C
import kaolin.ops.spc as spc_ops
from .interpolate import pytorch_interpolate, pytorch_interpolate_3d
...
def interpolate(coords, codebook_bitwidth, num_splashes, lod_idx, use_cuda, codebook, normalize):
    batch, n_coords = coords.shape
    feature_dim = codebook.feats.shape[-1]
    num_lods = codebook.resolutions.shape[0]

    if n_coords == 3:
        feats = pytorch_interpolate_3d(coords.contiguous(), codebook.feats, codebook.pos, codebook.var,
                                codebook.begin_idxes, codebook.begin_idxes_pos, codebook.resolutions, codebook_bitwidth, 
                                num_splashes, normalize)
        feats = feats.contiguous()
    else:   
        output = pytorch_interpolate(coords.contiguous(), codebook.feats, codebook.pos, codebook.var,
                                codebook.begin_idxes, codebook.begin_idxes_pos, codebook.resolutions, codebook_bitwidth, 
                                codebook.num_splashes_lod, normalize)
        feats = output[0].contiguous()
        gmms = output[1]

    return feats.reshape(batch, feature_dim*num_lods), gmms

class SplashGridInterpolate(torch.autograd.Function):
    @staticmethod
    @torch.cuda.amp.custom_fwd
    def forward(ctx, coords, resolutions, codebook_bitwidth, num_splashes, normalize, lod_idx, codebook, positions, variances, codebook_first_idx, codebook_first_idx_pos):
        ...
        feats_out = wisp_C.ops.splash_interpolate_cuda(coords.contiguous(), 
                                                         codebook,
                                                         positions,
                                                         variances,
                                                         codebook_first_idx,
                                                         codebook_first_idx_pos,
                                                         resolutions,
                                                         codebook_bitwidth,
                                                         num_splashes,
                                                         normalize).contiguous()
    
        ctx.save_for_backward(coords, codebook, positions, variances, codebook_first_idx, codebook_first_idx_pos)
    ...
    def backward(ctx, grad_output):
        ...
        positions = ctx.saved_tensors[2]
        variances = ctx.saved_tensors[3]
        codebook_first_idx = ctx.saved_tensors[4]
        codebook_first_idx_pos = ctx.saved_tensors[5]
        resolutions = ctx.resolutions
        feature_dim = ctx.feature_dim
        codebook_bitwidth = ctx.codebook_bitwidth
        num_splashes = ctx.num_splashes
        normalize = ctx.normalize
        coords_requires_grad = ctx.needs_input_grad[0]

        grad_coords, grad_codebook, grad_positions = wisp_C.ops.splash_interpolate_backward_cuda(
                coords.float().contiguous(), grad_output.contiguous(), codebook, positions, variances,
                codebook_first_idx,
                codebook_first_idx_pos,
                resolutions, 
                codebook_bitwidth, num_splashes, normalize, coords_requires_grad)
        
        if coords_requires_grad:
            return (grad_coords, None, None, None, None, None, grad_codebook, grad_positions, None, None, None)
        else:
            return (None, None, None, None, None, None, grad_codebook, grad_positions, None, None, None) 

def splashgrid(coords, codebook_bitwidth, num_splashes, lod_idx, codebook, normalize):
    batch, dim = coords.shape
    feats = SplashGridInterpolate.apply(coords.contiguous(), codebook.resolutions,
                                        codebook_bitwidth, num_splashes, normalize, lod_idx, codebook.feats, codebook.pos, codebook.var, codebook.begin_idxes, codebook.begin_idxes_pos)
    feature_dim = codebook.feature_dim * len(codebook.resolutions)
